...
/* Ze noemen mij ook wel blauwdruk */
struct data_header_t {
		int device_id;
		int sensor_value;
		char extra_message[25];
};
...
int main() {
		...
		struct data_header_t client_data_struct;			/* WOW EEN STRUCT WAUWIES!!!!11!!1 */
		...
		message_size = read(newsockfd, (void*) &client_data_struct, sizeof(client_data_struct));   /* Lees niet meer dan de grootte van de struct, om buffer overflows te voorkomen */
		...
		printf("Bericht voor u b0ss! We kijken even of er ook een bericht in zit voor u... 1sec.");
		if(strlen(client_data_struct.extra_message) != 0) {
				printf("Extra bericht: %s\n", client_data_struct.extra_message);
		}
